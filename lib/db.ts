...
// ── Shared Plant Feed ─────────────────────────────────────────
// Fixed: explicitly filters by org memberIds instead of relying on RLS joins.
// This ensures your OWN posts always appear and colleagues' posts appear
// without any RLS policy interference.
export async function getPlantFeed(
  ...
  // Step 2 — get this user's org_id (simple own-row query, always works)
  ...
  // Step 3 — build the list of user_ids to show in feed
  // Always include self. If in an org, include all org members.
  ...
  // Step 4 — fetch all three tables filtered by memberIds explicitly
  // No cross-table joins inside RLS-guarded queries — profiles fetched separately
  const [faultsRes, activitiesRes, shiftsRes] = await Promise.all([
    supabase
      .from('faults')
      .select('id, title, severity, status, detected_at, created_at, user_id, equipment:equipment(id,tag_id,name), fault_category:fault_categories(id,name,icon,color)')
    ...
    supabase
      .from('activities')
      .select('id, title, status, scheduled_date, created_at, user_id, equipment:equipment(id,tag_id,name), activity_type:activity_types(id,name,icon,color)')
      .in('user_id', memberIds)
      .order('created_at', { ascending: false })
      .limit(limit),
    ...
    supabase
      .from('shift_logs')
      .select('id, shift_type, shift_date, summary, handover_notes, open_issues, created_at, user_id, logged_by_name')
      .in('user_id', memberIds)
      .order('created_at', { ascending: false })
      .limit(20),
  ]);

  // Step 5 — fetch profiles separately and build a lookup map
  ...
  // Step 6 — assemble FeedItems with profile attached from the map
  const faultItems: FeedItem[] = (faultsRes.data || []).map((f: any) => ({
    id: f.id, type: 'fault' as const,
    created_at: f.created_at, user_id: f.user_id,
    profile: profileMap[f.user_id] ?? null,
    fault: f,
  }));

  const activityItems: FeedItem[] = (activitiesRes.data || []).map((a: any) => ({
  ...
  const shiftItems: FeedItem[] = (shiftsRes.data || []).map((s: any) => ({
    id: s.id, type: 'shift_log' as const,
