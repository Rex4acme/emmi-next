...
export default function FaultDetailPage() {
  ...
  return (
    ...
    >
      ...
      {/* Measurements */}
      {fault.measurements && Object.keys(fault.measurements).length > 0 && (
        <div className="card mb-4">
          <h3 className="text-xs font-bold uppercase tracking-widest mb-2" style={{ color: 'var(--text-3)' }}>Measurements</h3>
          <div className="grid grid-cols-2 gap-2">
            {Object.entries(fault.measurements).map(([k, v]) => (
              <div key={k} className="p-2 rounded" style={{ background: 'var(--surface)' }}>
                <p className="text-xs" style={{ color: 'var(--text-3)' }}>{k}</p>
                <p className="text-sm font-mono font-semibold" style={{ color: 'var(--amber)' }}>{v as string}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="card mb-4" style={{ border: '1px solid rgba(163,113,247,0.25)' }}>
        ...
        {aiLoading && (
          <div className="flex items-center gap-3 py-4">
            <div className="loading-dots"><span/><span/><span/></div>
            <span className="text-xs" style={{ color: 'var(--text-2)' }}>Claude is analysing…</span>
          </div>
        )}

        {!aiResult && !aiLoading && (
          <p className="text-xs" style={{ color: 'var(--text-3)' }}>
            Tap Analyse to get AI-powered root cause analysis and recommended actions from Claude Sonnet.
          </p>
        )}
      ...
      <div>
        ...
        {resolutions.length === 0 ? (
          ...
          <div className="space-y-3">
            {resolutions.map(res => (
              <Link key={res.id} href={`/faults/${fault.id}/resolution/${res.id}`}>
